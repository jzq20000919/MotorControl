...
void motor_ui_input_event(lv_event_t *event)
{
    ...
    *s_context.speed_command_tick = lv_tick_get();
    ...
    motor_link_set_speed_rpm(*s_context.pending_speed_rpm);
}
...
void motor_ui_mqtt_motor_event(lv_event_t *event)
{
    ...
    motor_link_get_snapshot(&snapshot);
    char payload[160];
    snprintf(payload, sizeof(payload), "running=%u mode=%s speed=%d position=%u.%02u", snapshot.motor_running ? 1U : 0U, snapshot.mode == MOTOR_LINK_MODE_SPEED ? "speed" : "position", snapshot.measured_speed_rpm, snapshot.current_position_cdeg / 100U, snapshot.current_position_cdeg % 100U);
    ...
    if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        if (*s_context.position_dragging) {
            ...
            *s_context.position_command_tick = lv_tick_get();
            motor_link_set_mode(MOTOR_LINK_MODE_POSITION);
            motor_link_set_position_cdeg(*s_context.pending_position_cdeg);
        }
        *s_context.position_dragging = false;
    }
}
