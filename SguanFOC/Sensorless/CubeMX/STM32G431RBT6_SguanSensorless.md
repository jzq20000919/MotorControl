# STM32G431RBT6 CubeMX configuration for sensorless SguanFOC

Generate the STM32 HAL project before importing this port. Add all
`Sensorless/Library/*.c` files and `Sensorless/Port/Src/*.c`, then add
`Sensorless/Library` and `Sensorless/Port/Inc` to the include paths.
...
`SGUAN_SENSORLESS_TIM1_ARR` (5312) in the port header. Set the CH4 compare to
...
`Example/stm32g4xx_it_sguan_sensorless.c` with CubeMX generated interrupts;
