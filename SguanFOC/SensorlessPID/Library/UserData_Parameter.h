...
static inline void User_Parameter_Init(SguanFOC_System_STRUCT *user){
    ...
    // 4.响应倍数参数
    user->value.Response = 16;                      // 16 x 62.5 us = 1 ms speed-loop period
    ...
    /* X-STAR-S board override: Rs=2.55 ohm, Ld/Lq=0.86 mH, 16 kHz PWM.
     * The speed loop output is an Iq request, so keep first commissioning
     * below the two-ampere motor rating.
     */
    /* PID values requested from the MCSDK tuning screen. */
    user->transfer.Current_D.Kp = 28.3828125f;       // 3633 / 128
    user->transfer.Current_D.Ki = 5.259765625f;      // 2693 / 512
    user->transfer.Current_D.Kd = 0.0f;
    user->transfer.Current_Q.Kp = 28.3828125f;       // 3633 / 128
    user->transfer.Current_Q.Ki = 5.259765625f;      // 2693 / 512
    user->transfer.Current_Q.Kd = 0.0f;
    user->transfer.Velocity.Kp = 1.046875f;          // 2144 / 2048
    user->transfer.Velocity.Ki = 0.00030517578125f;  // 5 / 16384
    user->transfer.Velocity.Kd = 0.0f;
    user->transfer.Velocity.Wc = 100.0f;
    user->transfer.Velocity.OutMax = 0.80f;
    user->transfer.Velocity.OutMin = -0.80f;
    user->value.Sensorless_Start = 130.0f;
    user->value.Sensorless_AbsMin = 140.0f;
    user->value.Sensorless_AbsMax = 155.0f;
}
