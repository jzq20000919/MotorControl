...
static inline void User_Initial_Init(void){
    SguanSensorlessPID_Port_InitialInit();
...
static inline void User_StartMotor_Init(void){
    SguanSensorlessPID_Port_StartMotor();
...
static inline void User_Delay(uint32_t ms){
    SguanSensorlessPID_Port_DelayMs(ms);
...
static inline int32_t User_ReadADC_Raw(int32_t Current_CH){
    return SguanSensorlessPID_Port_ReadPhaseCurrentRaw((uint8_t)Current_CH);
...
static inline void User_PwmDuty_Set(uint32_t Duty_u,
                                uint32_t Duty_v,
                                uint32_t Duty_w){
    SguanSensorlessPID_Port_SetPwmDuty(Duty_u, Duty_v, Duty_w);
